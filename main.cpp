...
//        cout << "i: "<< i<< endl;
        int point_size = (int)seg_global[i].size() * 2;
...
//            cout << "segment_id : " << seg->getClusterId() << endl;
    ...
    int cluster_cnt_size = (int)cluster_cnt.size();
    //cout << cluster_cnt_size <<" id  "<< cluster_id<< endl;

    for (int i = 0; i < cluster_cnt_size ; i++) {
        if (cluster_cnt[i].size() < min_traj) {
